...
namespace Confuser.Renamer
{
    internal class AnalyzePhase : ProtectionPhase
    {
        ...
        private void RegisterRenamers(ConfuserContext context, NameService service)
        {
            ...
            foreach(var asmRef in module.GetAssemblyRefs())
                if(asmRef.Name == "WindowsBase" || asmRef.Name == "PresentationCore" ||
                   asmRef.Name == "PresentationFramework" || asmRef.Name == "System.Xaml") wpf = true;
                else if(asmRef.Name == "Caliburn.Micro") caliburn = true;
                else if(asmRef.Name == "System.Windows.Forms") winforms = true;
                else if(asmRef.Name == "Newtonsoft.Json") json = true;
        ...
        {
            if(IsVisibleOutside(context, parameters, type)) service.SetCanRename(type, false);
            else if(type.IsRuntimeSpecialName || type.IsGlobalModuleType) service.SetCanRename(type, false);
            else if(type.FullName == "StellaObfuscator") service.SetCanRename(type, false);

            if(parameters.GetParameter(context, type, "forceRen", false))
                return;

            if(type.InheritsFromCorlib("System.Attribute")) service.ReduceRenameMode(type, RenameMode.ASCII);

            if(type.InheritsFrom("System.Configuration.SettingsBase")) service.SetCanRename(type, false);
        }
