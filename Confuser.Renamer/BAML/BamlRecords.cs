...
namespace Confuser.Renamer.BAML
{
    ...
    {
        public abstract BamlRecordType Type
        {
            get;
        }
    ...
    internal abstract class SizedBamlRecord : BamlRecord
    {
        ...
        {
            if((val & ~0x7F) == 0) return 1;
            if((val & ~0x3FFF) == 0) return 2;
            if((val & ~0x1FFFFF) == 0) return 3;
            if((val & ~0xFFFFFFF) == 0) return 4;
            return 5;
        }
    ...
    internal class NamedElementStartRecord : ElementStartRecord
    {
        ...
        {
            writer.Write(TypeId);
            if(RuntimeName != null) writer.Write(RuntimeName);
        }
    }
}