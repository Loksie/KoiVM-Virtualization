...
namespace Confuser.Core
{
    ...
    public class ObfAttrMarker : Marker
    {
        ...
        private bool ToInfo(ObfuscationAttributeInfo attr, out ProtectionSettingsInfo info)
        {
            ...
            if(info.Exclude && (!string.IsNullOrEmpty(attr.FeatureName) || !string.IsNullOrEmpty(attr.FeatureValue))) throw new ArgumentException("Feature property cannot be set when Exclude is true. Owner=" + attr.Owner);
            return true;
        }
        ...
        private void MarkModule(ProjectModule projModule, ModuleDefMD module, Rules rules, bool isMain)
        {
            ...
            if(project.Debug) module.LoadPdb();
        ...
        private void ProcessTypeMembers(TypeDef type, ProtectionSettingsStack stack)
        {
            ...
            foreach(var field in type.Fields) ProcessMember(field, stack);
        }

        private void ProcessMember(IDnlibDef member, ProtectionSettingsStack stack)
        {
        ...
        private class ProtectionSettingsStack
        {
            ...
            private void ApplyInfo(IDnlibDef context, ProtectionSettings settings, IEnumerable<ProtectionSettingsInfo> infos, ApplyInfoType type)
            {
                foreach(var info in infos)
                {
                    ...
                    if(info.Condition == null && info.Exclude)
                        if(type == ApplyInfoType.CurrentInfoOnly ||
                           type == ApplyInfoType.CurrentInfoInherits && info.ApplyToMember) settings.Clear();
                    if(!string.IsNullOrEmpty(info.Settings))
                        if(type == ApplyInfoType.ParentInfo && info.Condition != null && info.ApplyToMember ||
                           ...
                           type == ApplyInfoType.CurrentInfoInherits && info.Condition == null && info.ApplyToMember) parser.ParseProtectionString(settings, info.Settings);
                }
            }
