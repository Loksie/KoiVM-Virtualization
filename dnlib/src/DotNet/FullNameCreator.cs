...
namespace dnlib.DotNet
{
    ...
    public interface IFullNameCreatorHelper
    {
        ...
        /// </summary>
        /// <param name="type">
        ///     The type (<c>TypeDef</c>, <c>TypeRef</c> or <c>ExportedType</c>)
        ///     or <c>null</c>
        /// </param>
    ...
    public struct FullNameCreator
    {
        ...
        private string Result => sb == null ? null : sb.ToString();
        ...
        private void CreateTypeSigName(TypeSig typeSig, int flags)
        {
            ...
            var createNamespace = (flags & TYPESIG_NAMESPACE) != 0;
            var createName = (flags & TYPESIG_NAME) != 0;
                        ...
                        if(rank == 0)
                            ...
                            for(var i = 0; i < (int) rank; i++)
                            {
                                ...
                                if(!isReflection)
                                {
                                    ...
                                    var lower = arraySig.LowerBounds.Get(i, NO_LOWER);
                                    var size = arraySig.Sizes.Get(i, NO_SIZE);
                                    if(lower != NO_LOWER)
                                    {
                                        sb.Append(lower);
                                        sb.Append("..");
                    ...
                    if(createNamespace && createName)
                        if(isReflection)
                        {
                            ...
                            foreach(var genArg in typeGenArgs.GetSafeEnumerable())
                            {
                                ...
                                var mustWriteAssembly = MustUseAssemblyName(genArg);
        ...
        private ModuleDef GetOwnerModule(TypeDef typeDef)
        {
            ...
            for(var i = recursionCounter.Counter; i < RecursionCounter.MAX_RECURSION_COUNT; i++)
            {
        ...
        private void CreateMethodFullName(string declaringType, string name, MethodBaseSig methodSig, MethodDef gppMethod)
        {
            ...
            {
                sb.Append('<');
                var genParamCount = methodSig.GenParamCount;
            ...
            sb.Append('(');
            var count = PrintMethodArgList(methodSig.Params, false, false);
