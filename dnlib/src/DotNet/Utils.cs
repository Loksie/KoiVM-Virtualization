...
namespace dnlib.DotNet
{
    ...
    internal static class Utils
    {
        ...
        internal static string GetAssemblyNameString(UTF8String name, Version version, UTF8String culture, PublicKeyBase publicKey, AssemblyAttributes attributes)
        {
            ...
            {
                sb.Append(", Version=");
                sb.Append(CreateVersionWithNoUndefinedValues(version));
            }
        ...
        /// <returns>
        ///     <paramref name="hexString" /> converted to a byte[] or <c>null</c>
        ///     if <paramref name="hexString" /> is invalid
        /// </returns>
        internal static byte[] ParseBytes(string hexString)
        {
            try
            {
                ...
                {
                    var upper = TryParseHexChar(hexString[i]);
                    var lower = TryParseHexChar(hexString[i + 1]);
                    if(upper < 0 || lower < 0)
                        return null;
        ...
        /// <returns>
        ///     <c>0x00</c>-<c>0x0F</c> if successful, <c>-1</c> if <paramref name="c" /> is not
        ///     a valid hex digit
        /// </returns>
        ...
        internal static int GetHashCode(byte[] a)
        {
            if(a == null || a.Length == 0)
                return 0;
            var count = Math.Min(a.Length / 2, 20);
            if(count == 0)
                count = 1;
            uint hash = 0;
        ...
        /// </summary>
        /// <remarks>
        ///     This differs from <see cref="System.Version.CompareTo(Version)" /> if the build
        ...
        /// </summary>
        /// <remarks>
        ///     This differs from <see cref="System.Version.Equals(Version)" /> if the build
        ///     and/or revision numbers haven't been initialized or if one of the args is <c>null</c>.
        /// </remarks>
        /// <param name="a">Version #1 or <c>null</c> to be treated as v0.0.0.0</param>
        /// <param name="b">Version #2 or <c>null</c> to be treated as v0.0.0.0</param>
        ...
        /// <returns>
        ///     A new <see cref="Version" /> or <c>null</c> if <paramref name="versionString" />
