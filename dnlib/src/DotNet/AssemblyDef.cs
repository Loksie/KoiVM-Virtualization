    ...
    public abstract class AssemblyDef : IHasCustomAttribute, IHasDeclSecurity, IAssembly, IListListener<ModuleDef>, ITypeDefFinder, IDnlibDef
    {
        ...
        public static AssemblyDef Load(byte[] data, ModuleCreationOptions options)
        {
            ...
            try
            {
                ...
                var asm = module.Assembly;
                if(asm == null)
                    throw new BadImageFormatException(string.Format("{0} is only a .NET module, not a .NET assembly. Use ModuleDef.Load().", module));
                return asm;
            }
            catch
            {
        ...
        public static AssemblyDef Load(IntPtr addr, ModuleCreationOptions options)
        {
            ...
            try
            {
                ...
                var asm = module.Assembly;
                if(asm == null)
                    throw new BadImageFormatException(string.Format("{0} (addr: {1:X8}) is only a .NET module, not a .NET assembly. Use ModuleDef.Load().", module, addr.ToInt64()));
                return asm;
            }
            catch
            {
    ...
    internal sealed class AssemblyDefMD : AssemblyDef, IMDTokenProviderMD
    {
        ...
        public AssemblyDefMD(ModuleDefMD readerModule, uint rid)
        {
            ...
            if(rid != 1)
                modules = new LazyList<ModuleDef>(this);
            uint publicKey, name;
            var culture = readerModule.TablesStream.ReadAssemblyRow(OrigRid, out hashAlgorithm, out version, out attributes, out publicKey, out name);
