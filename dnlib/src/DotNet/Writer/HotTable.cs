...
namespace dnlib.DotNet.Writer
{
    ...
    public abstract class HotTable : IChunk
    {
        ...
        /// </summary>
        private uint FullTableSize => (uint) (mdTable.Rows * mdTable.TableInfo.RowSize);
        ...
        /// </summary>
        /// <param name="rid">
        ///     The row ID. This must be the new rid, so this method can only be
        ///     called after the table row has been assigned a new rid.
        /// </param>
        ...
        {
            mdTable.SetReadOnly();
            var maxBits = CountMaxBits((uint) mdTable.Rows);
            if(maxBits >= 16)
                return maxBits - 8;
            return maxBits / 2;
        }
        ...
        private void CreateFirstAndSecondLevelTables()
        {
            ...
            for(var i = 0; i < secondLevelTable.Length; i++)
    ...
    public sealed class HotTable40 : HotTable
    {
        ...
        {
            for(var i = 0; i < partialData.Count; i++)
