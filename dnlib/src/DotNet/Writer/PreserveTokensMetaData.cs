    ...
    internal sealed class PreserveTokensMetaData : MetaData
    {
        ...
        protected override List<TypeDef> GetAllTypeDefs()
        {
            ...
            types.Sort((a, b) =>
            {
                ...
                var ai = typeToIndex[a];
                var bi = typeToIndex[b];
                var amd = (ai & IS_TYPEDEFMD) != 0;
                var bmd = (bi & IS_TYPEDEFMD) != 0;
                if(amd == bmd)
                {
            ...
            for(var i = 1; i < types.Count; i++)
            {
                ...
                var currRid = type.Rid;
                var extraTypes = (int) (currRid - prevRid - 1);
                if(extraTypes != 0)
                    for(var j = 0; j < extraTypes; j++)
        ...
        protected override void AllocateTypeDefRids()
        {
            ...
            {
                var rid = tablesHeap.TypeDefTable.Create(new RawTypeDefRow());
        ...
        private void InitializeTypeRefTableRows()
        {
            ...
            var rows = mod.TablesStream.TypeRefTable.Rows;
        ...
        private void InitializeMemberRefTableRows()
        {
            ...
            var rows = mod.TablesStream.MemberRefTable.Rows;
            for(uint rid = 1; rid <= rows; rid++)
        ...
        private void InitializeStandAloneSigTableRows()
        {
            ...
            var rows = mod.TablesStream.StandAloneSigTable.Rows;
            for(uint rid = 1; rid <= rows; rid++)
        ...
        private void InitializeTypeSpecTableRows()
        {
            ...
            var rows = mod.TablesStream.TypeSpecTable.Rows;
            for(uint rid = 1; rid <= rows; rid++)
        ...
        private void InitializeMethodSpecTableRows()
        {
            ...
            var rows = mod.TablesStream.MethodSpecTable.Rows;
            for(uint rid = 1; rid <= rows; rid++)
        ...
        protected override void AllocateMemberDefRids()
        {
            ...
            {
                for(var i = 0; i < fieldDefInfos.Count; i++)
                {
            ...
            {
                for(var i = 0; i < methodDefInfos.Count; i++)
                {
            ...
            if(paramDefInfos.NeedPtrTable)
            {
                ...
                for(var i = 0; i < paramDefInfos.Count; i++)
                {
            ...
            if(eventDefInfos.NeedPtrTable)
                for(var i = 0; i < eventDefInfos.Count; i++)
                {
            ...
            if(propertyDefInfos.NeedPtrTable)
                for(var i = 0; i < propertyDefInfos.Count; i++)
                {
        ...
        private void ReUseDeletedFieldRows()
        {
            ...
            var fieldSig = GetSignature(new FieldSig(module.CorLibTypes.Byte));
            for(var i = 0; i < hasOwner.Length; i++)
            {
                if(hasOwner[i])
                    continue;
                var frid = (uint) i + 1;
        ...
        private void ReUseDeletedMethodRows()
        {
            ...
            var methodSig = GetSignature(MethodSig.CreateInstance(module.CorLibTypes.Void));
            for(var i = 0; i < hasOwner.Length; i++)
            {
                if(hasOwner[i])
                    continue;
        ...
        private void ReUseDeletedParamRows()
        {
            ...
            for(var i = 0; i < hasOwner.Length; i++)
            {
                ...
                var ptrRid = tablesHeap.ParamPtrTable.Create(new RawParamPtrRow(prid));
                ...
                var mrow = new RawMethodRow(0,
                    ...
                    methodSig,
                    ptrRid);
                var mrid = tablesHeap.MethodTable.Create(mrow);
        ...
        private void ReUseDeletedEventRows()
        {
            ...
            var typeRid = CreateDummyPtrTableType();
            tablesHeap.EventMapTable.Create(new RawEventMapRow(typeRid, (uint) tablesHeap.EventPtrTable.Rows + 1));

            var eventType = AddTypeDefOrRef(module.CorLibTypes.Object.TypeDefOrRef);
            for(var i = 0; i < hasOwner.Length; i++)
            {
                if(hasOwner[i])
                    continue;
        ...
        private void ReUseDeletedPropertyRows()
        {
            ...
            var propertySig = GetSignature(PropertySig.CreateStatic(module.CorLibTypes.Object));
            for(var i = 0; i < hasOwner.Length; i++)
            {
                if(hasOwner[i])
                    continue;
        ...
        private uint CreateDummyPtrTableType()
        {
            ...
            var numFields = fieldDefInfos.NeedPtrTable ? fieldDefInfos.Count : fieldDefInfos.TableSize;
            var numMethods = methodDefInfos.NeedPtrTable ? methodDefInfos.Count : methodDefInfos.TableSize;
        ...
        private void InitializeEventMap()
        {
            ...
            for(var i = 0; i < eventDefInfos.Count; i++)
            {
                ...
                var eventMapRid = tablesHeap.EventMapTable.Create(row);
        ...
        private void InitializePropertyMap()
        {
            ...
            for(var i = 0; i < propertyDefInfos.Count; i++)
            {
                ...
                var propertyMapRid = tablesHeap.PropertyMapTable.Create(row);
        ...
        protected override uint AddTypeRef(TypeRef tr)
        {
            ...
            var isOld = PreserveTypeRefRids && mod.ResolveTypeRef(tr.Rid) == tr;
        ...
        private uint AddTypeSpec(TypeSpec ts, bool forceIsOld)
        {
            ...
            var isOld = forceIsOld || PreserveTypeSpecRids && mod.ResolveTypeSpec(ts.Rid) == ts;
        ...
        private uint AddMemberRef(MemberRef mr, bool forceIsOld)
        {
            ...
            var isOld = forceIsOld || PreserveMemberRefRids && mod.ResolveMemberRef(mr.Rid) == mr;
        ...
        private uint AddStandAloneSig(StandAloneSig sas, bool forceIsOld)
        {
            ...
            var isOld = forceIsOld || PreserveStandAloneSigRids && mod.ResolveStandAloneSig(sas.Rid) == sas;
        ...
        public override MDToken GetToken(IList<TypeSig> locals, uint origToken)
        {
            ...
            var rid = AddStandAloneSig(new LocalSig(locals, false), origToken);
            if(rid == 0)
                return base.GetToken(locals, origToken);
        ...
        protected override uint AddStandAloneSig(MethodSig methodSig, uint origToken)
        {
            ...
            var rid = AddStandAloneSig(methodSig, origToken);
            if(rid == 0)
                return base.AddStandAloneSig(methodSig, origToken);
            return rid;
        }
        ...
        {
            var sig = GetSignature(callConvSig);
            uint otherSig;
            ...
            var rid = MDToken.ToRID(origToken);
        ...
        private uint AddMethodSpec(MethodSpec ms, bool forceIsOld)
        {
            ...
            var isOld = forceIsOld || PreserveMethodSpecRids && mod.ResolveMethodSpec(ms.Rid) == ms;
        ...
        private sealed class MemberDefDict<T> where T : IMDTokenProvider
        {
            ...
            public void Sort(Comparison<MemberDefInfo<T>> comparer)
            {
                ...
                wasSorted = true;
                for(var i = 0; i < sortedDefs.Count; i++)
                {
            ...
            public void SortDefs()
            {
                ...
                if(preserveRids)
                {
                    ...
                    var newRid = numDefMDs == 0 ? 1 : defs[numDefMDs - 1].Rid + 1;
                    for(var i = numDefMDs; i < defs.Count; i++)
                        defs[i].Rid = newRid++;
