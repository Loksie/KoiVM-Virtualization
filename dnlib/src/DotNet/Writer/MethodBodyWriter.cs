...
namespace dnlib.DotNet.Writer
{
    ...
    public interface ITokenCreator : IWriterError
    {
        ...
        /// <returns>
        ///     A <c>StandAloneSig</c> token or <c>0</c> if <paramref name="locals" /> is
        ///     empty.
        /// </returns>
    ...
    public sealed class MethodBodyWriter : MethodBodyWriterBase
    {
        ...
        {
            var padding = Utils.AlignUp(Code.Length, 4) - Code.Length;
        ...
        private IList<TypeSig> GetLocals()
        {
            ...
            for(var i = 0; i < cilBody.Variables.Count; i++)
        ...
        {
            var offs1 = GetOffset2(start);
            var offs2 = GetOffset2(end);
            if(offs2 < offs1)
                return false;
        ...
        private void WriteFatExceptionClauses(BinaryWriter writer)
        {
            ...
            var numExceptionHandlers = exceptionHandlers.Count;
