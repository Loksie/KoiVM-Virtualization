...
namespace dnlib.DotNet.Writer
{
    ...
    public abstract class MetaData : IChunk, ISignatureWriterHelper, ITokenCreator, ICustomAttributeWriterHelper
    {
        ...
        internal MetaData(ModuleDef module, UniqueChunkList<ByteArrayChunk> constants, MethodBodyChunks methodBodies, NetResources netResources, MetaDataOptions options)
        {
            ...
            metaDataHeader = new MetaDataHeader(this.options.MetaDataHeaderOptions);
            tablesHeap = new TablesHeap(this.options.TablesHeapOptions);
        ...
        /// </summary>
        internal byte[] AssemblyPublicKey
        {
            get;
            set;
        }
        ...
        /// </summary>
        protected abstract int NumberOfMethods
        {
            get;
        }
        ...
        public void SetOffset(FileOffset offset, RVA rva)
        {
            ...
            var len = metaDataHeader.GetFileLength();
            offset += len;
            rva += len;
        ...
        public virtual MDToken GetToken(IList<TypeSig> locals, uint origToken)
        {
            ...
            var rid = tablesHeap.StandAloneSigTable.Add(row);
        ...
        /// <returns>
        ///     The <see cref="MethodBody" /> or <c>null</c> if <paramref name="md" /> is
        ///     <c>null</c> or not a method defined in this module.
        /// </returns>
        ...
        /// <returns>
        ///     A <see cref="ByteArrayChunk" /> instance or <c>null</c> if <paramref name="er" />
        ///     is invalid
        /// </returns>
        ...
        /// <returns>
        ///     A <see cref="ByteArrayChunk" /> instance or <c>null</c> if <paramref name="fd" />
        ///     is invalid
        /// </returns>
        ...
        {
            var numMethods = NumberOfMethods;
            var methodNum = 0;
            var notifyNum = 0;
            const int numNotifyEvents = 10; // WriteMethodBodies0 - WriteMethodBodies9
            var notifyAfter = numMethods / numNotifyEvents;

            var keepMaxStack = KeepOldMaxStack;
            foreach(var type in allTypeDefs)
            {
                if(type == null)
                    continue;
        ...
        protected void AddNestedType(TypeDef nestedType, TypeDef declaringType)
        {
            if(nestedType == null || declaringType == null)
                return;
            var nestedRid = GetRid(nestedType);
            var dtRid = GetRid(declaringType);
        ...
        private void AddGenericParamConstraints(IList<GenericParam> gps)
        {
            ...
            foreach(var gp in gps)
            {
                if(gp == null)
                    continue;
                var rid = genericParamInfos.Rid(gp);
        ...
        private void AddMethodImpls(MethodDef method, IList<MethodOverride> overrides)
        {
            ...
            var rid = GetRid(method.DeclaringType);
        ...
        ///     <see cref="PreserveExtraSignatureData" /> is <c>true</c>.
        /// </param>
        ...
        private void AppendExtraData(ref byte[] blob, byte[] extraData)
        {
            ...
            {
                var blen = blob == null ? 0 : blob.Length;
        ...
        internal sealed class SortedRows<T, TRow> where T : class where TRow : class
        {
            ...
            private readonly Dictionary<T, uint> toRid = new Dictionary<T, uint>();
