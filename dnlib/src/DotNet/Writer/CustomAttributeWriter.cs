...
namespace dnlib.DotNet.Writer
{
    ...
    public struct CustomAttributeWriter : IDisposable
    {
        ...
        private void Write(CustomAttribute ca)
        {
            ...
            var numArgs = Math.Min(methodSig.Params.Count, ca.ConstructorArguments.Count);
            for(var i = 0; i < numArgs; i++)
                WriteValue(FixTypeSig(methodSig.Params[i]), ca.ConstructorArguments[i]);

            var numNamedArgs = Math.Min(ushort.MaxValue, ca.NamedArguments.Count);
            writer.Write((ushort) numNamedArgs);
            for(var i = 0; i < numNamedArgs; i++)
        ...
        /// <returns>
        ///     A <see cref="TypeDef" /> or <c>null</c> if we couldn't resolve the
        ///     <see cref="TypeRef" /> or if <paramref name="type" /> is a type spec
        /// </returns>
        ...
        private void WriteFieldOrPropType(TypeSig type)
        {
            ...
            switch(type.ElementType)
            {
                ...
                case ElementType.Char:
                    ...
                    break;
                case ElementType.I1:
                    writer.Write((byte) SerializationType.I1);
                    break;
                case ElementType.U1:
                    writer.Write((byte) SerializationType.U1);
                    break;
                case ElementType.I2:
                    writer.Write((byte) SerializationType.I2);
                    break;
                case ElementType.U2:
                    writer.Write((byte) SerializationType.U2);
                    break;
                case ElementType.I4:
                    writer.Write((byte) SerializationType.I4);
                    break;
                case ElementType.U4:
                    writer.Write((byte) SerializationType.U4);
                    break;
                case ElementType.I8:
                    writer.Write((byte) SerializationType.I8);
                    break;
                case ElementType.U8:
                    writer.Write((byte) SerializationType.U8);
                    break;
                case ElementType.R4:
                    writer.Write((byte) SerializationType.R4);
                    break;
                case ElementType.R8:
                    writer.Write((byte) SerializationType.R8);
                    break;
                case ElementType.String:
