    ...
    internal sealed class NormalMetaData : MetaData
    {
        ...
        protected override void AllocateMemberDefRids()
        {
            ...
            foreach(var type in allTypeDefs)
            {
                ...
                {
                    var eventMapRid = tablesHeap.EventMapTable.Create(new RawEventMapRow(typeRid, eventListRid));
                    eventMapInfos.Add(type, eventMapRid);
                    foreach(var evt in type.Events)
                    {
                        if(evt == null)
                            continue;
                        var rid = eventListRid++;
                ...
                {
                    var propertyMapRid = tablesHeap.PropertyMapTable.Create(new RawPropertyMapRow(typeRid, propertyListRid));
                    propertyMapInfos.Add(type, propertyMapRid);
                    foreach(var prop in type.Properties)
                    {
                        if(prop == null)
                            continue;
                        var rid = propertyListRid++;
