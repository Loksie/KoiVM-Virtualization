...
namespace dnlib.DotNet.Writer
{
    ...
    internal struct SectionSizes
    {
        ...
        public SectionSizes(uint fileAlignment, uint sectionAlignment, uint headerLen, MFunc<IEnumerable<SectionSizeInfo>> getSectionSizeInfos)
        {
            ...
            {
                var sectAlignedVs = Utils.AlignUp(section.length, sectionAlignment);
                var fileAlignedVs = Utils.AlignUp(section.length, fileAlignment);

                var isCode = (section.characteristics & 0x20) != 0;
                var isInitdData = (section.characteristics & 0x40) != 0;
                var isUnInitdData = (section.characteristics & 0x80) != 0;
