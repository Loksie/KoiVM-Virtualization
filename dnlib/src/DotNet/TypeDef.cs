...
namespace dnlib.DotNet
{
    ...
    public abstract class TypeDef : ITypeDefOrRef, IHasCustomAttribute, IHasDeclSecurity, IMemberRefParent, ITypeOrMethodDef, IListListener<FieldDef>, IListListener<MethodDef>, IListListener<TypeDef>, IListListener<EventDef>, IListListener<PropertyDef>, IListListener<GenericParam>, IMemberRefResolver, IMemberDef
    {
        ...
        /// </summary>
        public ThreadSafe.IEnumerable<TypeDef> GetTypes()
        {
        ...
        /// <returns>
        ///     A <see cref="MethodDef" /> or a <see cref="FieldDef" /> instance or <c>null</c>
        ...
        public MethodDef FindMethod(UTF8String name, MethodSig sig, SigComparerOptions options, ModuleDef sourceModule)
        {
            ...
            var allowPrivateScope = (options & SigComparerOptions.PrivateScopeMethodIsComparable) != 0;
        ...
        public ThreadSafe.IEnumerable<MethodDef> FindMethods(UTF8String name)
        {
            foreach(var method in Methods.GetSafeEnumerable())
                if(UTF8String.Equals(method.Name, name))
                    yield return method;
        ...
        public MethodDef FindOrCreateStaticConstructor()
        {
            ...
            cctor.Body = body;
            return Methods.ExecuteLocked(cctor, (tsList, cctor2) =>
            {
        ...
        public ThreadSafe.IEnumerable<MethodDef> FindInstanceConstructors()
        {
            foreach(var method in Methods.GetSafeEnumerable())
        ...
        public ThreadSafe.IEnumerable<MethodDef> FindConstructors()
        {
            foreach(var method in Methods.GetSafeEnumerable())
        ...
        public FieldDef FindField(UTF8String name, FieldSig sig, SigComparerOptions options, ModuleDef sourceModule)
        {
            if(UTF8String.IsNull(name) || sig == null)
                return null;
            var comparer = new SigComparer(options, sourceModule);
            var allowPrivateScope = (options & SigComparerOptions.PrivateScopeFieldIsComparable) != 0;
            foreach(var field in Fields.GetSafeEnumerable())
            {
        ...
        public ThreadSafe.IEnumerable<FieldDef> FindFields(UTF8String name)
        {
            foreach(var field in Fields.GetSafeEnumerable())
                if(UTF8String.Equals(field.Name, name))
                    yield return field;
        ...
        public ThreadSafe.IEnumerable<EventDef> FindEvents(UTF8String name)
        {
            foreach(var @event in Events.GetSafeEnumerable())
                if(UTF8String.Equals(@event.Name, name))
                    yield return @event;
        ...
        public ThreadSafe.IEnumerable<PropertyDef> FindProperties(UTF8String name)
        {
            foreach(var prop in Properties.GetSafeEnumerable())
                if(UTF8String.Equals(prop.Name, name))
                    yield return prop;
        ...
        /// </summary>
        /// <param name="method">The method to remove</param>
        /// <param name="removeEmptyPropertiesEvents">
        ///     <c>true</c> if we should remove all
        ...
        public ThreadSafe.IList<FieldDef> GetFields(UTF8String name)
        {
            var fields = new ThreadSafe.List<FieldDef>();
            foreach(var field in Fields.GetSafeEnumerable())
                if(field.Name == name)
                    fields.Add(field);
            return fields;
        ...
        internal static bool GetClassSize(TypeDef td, out uint size)
        {
            ...
            if(classLayout == null)
                return false;
            var classSize = classLayout.ClassSize;
            if(classSize != 0)
            {
                size = classSize;
                return true;
    ...
    internal sealed class TypeDefMD : TypeDef, IMDTokenProviderMD
    {
        ...
        private ThreadSafe.Dictionary<uint, ThreadSafe.IList<MethodOverrideTokens>> methodRidToOverrides;
        ...
        public TypeDefMD(ModuleDefMD readerModule, uint rid)
        {
            ...
            uint name, @namespace;
            extendsCodedToken = readerModule.TablesStream.ReadTypeDefRow(OrigRid, out attributes, out name, out @namespace);
        ...
        {
            var enclosingClass = readerModule.TablesStream.ReadNestedClassRow2(readerModule.MetaData.GetNestedClassRid(OrigRid));
        ...
        {
            var newMethodRidToOverrides = new ThreadSafe.Dictionary<uint, ThreadSafe.IList<MethodOverrideTokens>>();

            var ridList = readerModule.MetaData.GetMethodImplRidList(OrigRid);
            for(uint i = 0; i < ridList.Length; i++)
            {
                uint methodBodyToken;
                var methodDeclToken = readerModule.TablesStream.ReadMethodImplRow(ridList[i], out methodBodyToken);
        ...
        internal void InitializeMethodSemanticsAttributes()
        {
            ...
            for(uint i = 0; i < list.Length; i++)
            {
                ...
                for(uint j = 0; j < ridList.Length; j++)
                {
                    ...
                    var methodToken = readerModule.TablesStream.ReadMethodSemanticsRow(ridList[j], out semantics);
        ...
        internal void InitializeProperty(PropertyDefMD prop, out ThreadSafe.IList<MethodDef> getMethods, out ThreadSafe.IList<MethodDef> setMethods, out ThreadSafe.IList<MethodDef> otherMethods)
        {
            ...
            {
                MethodSemanticsAttributes semantics;
                var methodToken = readerModule.TablesStream.ReadMethodSemanticsRow(ridList[i], out semantics);
                var method = readerModule.ResolveMethod(methodToken);
