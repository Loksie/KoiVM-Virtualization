...
namespace dnlib.DotNet
{
    ...
    public struct StrongNameSigner
    {
        ...
        ///     Offset in <paramref name="stream" /> of the first byte of
        ///     the PE file.
        /// </param>
        ...
        {
            var snSigSize = (uint) snk.SignatureSize;
        ...
        private byte[] GetStrongNameSignature(StrongNameKey snk, AssemblyHashAlgorithm hashAlg, byte[] hash)
        {
            using(var rsa = snk.CreateRSA())
            {
                ...
                var hashName = hashAlg.GetName() ?? AssemblyHashAlgorithm.SHA1.GetName();
