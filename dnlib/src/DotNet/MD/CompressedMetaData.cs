...
namespace dnlib.DotNet.MD
{
    ...
    {
        private static readonly string nativeImages40;
        ...
        protected override void InitializeInternal()
        {
            ...
            try
            {
                ...
                for(var i = mdHeader.StreamHeaders.Count - 1; i >= 0; i--)
                {
        ...
        private void InitializeHotStreams(IList<HotStream> hotStreams)
        {
            ...
            var offsetMask = GetPointerSize() == 8 ? -1L : uint.MaxValue;
            ...
            for(var i = hotStreams.Count - 1; i >= 0; i--)
            {
                ...
                // It's always the last one found that is used
                for(var j = hotHeapStreams.Count - 1; j >= 0; j--)
                {
        ...
        private RidList GetRidList(MDTable tableSource, uint tableSourceRid, int colIndex, MDTable tableDest)
        {
            ...
            if(startRid == 0 || startRid >= lastRid)
                return RidList.Empty;
            var endRid = hasNext ? nextListRid : lastRid;
            if(endRid < startRid)
                endRid = startRid;
            if(endRid > lastRid)
                endRid = lastRid;
