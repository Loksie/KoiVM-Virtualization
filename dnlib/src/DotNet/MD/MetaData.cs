...
namespace dnlib.DotNet.MD
{
    ...
    internal abstract class MetaData : IMetaData
    {
        ...
        private ThreadSafe.Dictionary<uint, RandomRidList> typeDefRidToNestedClasses;
...
#endif
            var startRid = BinarySearch_NoLock(tableSource, keyColIndex, key);
            if(tableSource.IsInvalidRID(startRid))
                return RidList.Empty;
            var endRid = startRid + 1;
        ...
        protected sealed class SortedTable
        {
            ...
            private void InitializeKeys(MDTable mdTable, int keyColIndex)
            {
                ...
                using(var reader = mdTable.CloneImageStream())
                {
                    ...
                    var increment = mdTable.TableInfo.RowSize - keyColumn.Size;
            ...
            {
                var startIndex = BinarySearch(key);
                if(startIndex == 0)
                    return RidList.Empty;
