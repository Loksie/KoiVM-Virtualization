...
namespace dnlib.DotNet.MD
{
    ...
    public sealed class CodedToken
    {
        ...
        {
            var index = Array.IndexOf(TableTypes, MDToken.ToTable(token));
            if(index < 0)
            {
        ...
        {
            uint decodedToken;
            var result = Decode(codedToken, out decodedToken);
        ...
        {
            var rid = codedToken >> Bits;
            var index = (int) (codedToken & mask);
