...
namespace dnlib.DotNet
{
    ...
    internal struct AssemblyHash : IDisposable
    {
        ...
        /// </summary>
        /// <remarks>
        ///     If <paramref name="hashAlgo" /> is an unsupported hash algorithm, then
        ///     <see cref="AssemblyHashAlgorithm.SHA1" /> will be used as the hash algorithm.
        /// </remarks>
        ...
        public void Hash(Stream stream, uint length, byte[] buffer)
        {
            while(length > 0)
            {
                var len = length > (uint) buffer.Length ? buffer.Length : (int) length;
        ...
        public static PublicKeyToken CreatePublicKeyToken(byte[] publicKeyData)
        {
            ...
            var pkt = new byte[8];
            for(var i = 0; i < pkt.Length && i < hash.Length; i++)
