...
/// <returns><c>false</c> to break out of the iterator loop and return</returns>
    ...
    public static partial class Extensions
    {
        ...
        /// <param name="handler">Handler that should execute when the lock is held</param>
        /// <returns>The value <paramref name="handler" /> returns</returns>
        public static TRetType ExecuteLocked<T, TArgType, TRetType>(this IList<T> tsList, TArgType arg, ExecuteLockedDelegate<T, TArgType, TRetType> handler)
        {
#if THREAD_SAFE
...
/// <param name="reverseOrder"><c>true</c> if we should iterate in the reverse order</param>
        ...
        public static void Iterate<T>(this IList<T> list, int startIndex, int endIndex, bool reverseOrder, ListIterateDelegate<T> handler)
        {
            ...
            if(reverseOrder)
            {
                var i = (endIndex < 0 ? list.Count : endIndex) - 1;
                for(; i >= startIndex; i--)
            ...
            {
                // Count property can change so check it each time in the loop
                for(var i = startIndex; i < (endIndex < 0 ? list.Count : endIndex); i++)
                    if(!handler(list, i, list[i]))
                        break;
            }
#if THREAD_SAFE
        ...
        /// <returns>
        ///     <c>true</c> if <paramref name="value" /> was updated with the element in the
        ///     list or <c>false</c> if <paramref name="index" /> was invalid.
        /// </returns>
        ...
        /// <returns>
        ///     The value in the list or <paramref name="defaultValue" /> if
        ///     <paramref name="index" /> was invalid
        /// </returns>
        ...
        /// <param name="value">Value</param>
        /// <returns>
        ///     <c>true</c> if <paramref name="value" /> was written to the list or <c>false</c>
        ///     if <paramref name="index" /> was invalid.
        /// </returns>
