...
namespace dnlib.IO
{
    ...
    /// </summary>
    /// <remarks>
    ///     Since this class maps a file into memory, the user should call
    ///     <see cref="UnmanagedMemoryStreamCreator.Dispose()" /> to free any resources
    ///     used by the class when it's no longer needed.
    /// </remarks>
    ...
    internal sealed class MemoryMappedFileStreamCreator : UnmanagedMemoryStreamCreator
    {
        ...
        /// </summary>
        /// <remarks>
        ///     If <paramref name="mapAsImage" /> is <c>true</c>, then the created
        ...
        ///     set <see cref="UnmanagedMemoryStreamCreator.Length" /> to the correct image length to access the full image.
        /// </remarks>
        ...
        private static class Windows
        {
            ...
            public static void Mmap(MemoryMappedFileStreamCreator creator, bool mapAsImage)
            {
                using(var fileHandle = CreateFile(creator.theFileName, GENERIC_READ, FILE_SHARE_READ, IntPtr.Zero, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, IntPtr.Zero))
                {
                    ...
                    uint sizeHi;
                    var sizeLo = GetFileSize(fileHandle, out sizeHi);
                    int hr;
        ...
        private static class Unix
        {
            ...
            public static void Mmap(MemoryMappedFileStreamCreator creator, bool mapAsImage)
            {
                var fd = open(creator.theFileName, O_RDONLY);
                try
                {
                    if(fd < 0)
